...
from my_token import TokenType, ARITHMETIC_TOKENS
...
class Compiler:
    ...
    def compile(self) -> tuple[Instruction, ...]:
        ...
        self.__patch_jump(jmp_instruction_address, len(self.__instructions))
    ...
    def __var_update_stmt(self, node: VarUpdate, scope):
        if node.name in scope.variables:
            cur_scope = scope
    ...
    def __if_stmt(self, node: IfStmt, scope):
        ...
        self.__patch_jump(condition_jmp_address, len(self.__instructions))
    ...
    def __while_stmt(self, node: WhileStmt, scope):
        ...
        for num, stmt in enumerate(node.block):
            if isinstance(stmt, BreakStmt):
                jmp_instructions_addresses[f"Break {num}"] = self.__emit(OpCode.JUMP)
            elif isinstance(stmt, ContinueStmt):
                jmp_instructions_addresses[f"Continue {num}"] = self.__emit(OpCode.JUMP)
